...
class Render_3DMM(nn.Module):
    ...
    def get_render(self, batch_size=1):
        ...
        sigma = 1e-4
        #此处可能出现：Bin size was too small in the coarse rasterization phase. This caused an overflow, meaning output may be incomplete. To solve, try increasing max_faces_per_bin / max_points_per_bin, decreasing bin_size, or setting bin_size to 0 to use the naive rasterization
        #可尝试设置bin_size=0
        #参考：https://github.com/facebookresearch/pytorch3d/issues/1064
        #源码参考：https://github.com/facebookresearch/pytorch3d/blob/c2862ff42706ae98ce4053e7d76959f05fb2c3b3/pytorch3d/renderer/mesh/rasterizer.py#L37-L45
        # 参数：bin_size=0 不可设置为0，否则巨慢无比！！！
