...
# wav2vec2_processor = Wav2Vec2Processor.from_pretrained("facebook/hubert-large-ls960-ft")
# Ajian 20231025 修改为加载本地模型文件，模型文件在https://huggingface.co/facebook/hubert-large-ls960-ft/tree/main下载
# 该修改适用于AutoDL平台
start = time.time()
device = "cuda:0"
try:
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "facebook/")
    wav2vec2_processor = Wav2Vec2FeatureExtractor.from_pretrained(path)
    hubert_model = HubertModel.from_pretrained(path)
