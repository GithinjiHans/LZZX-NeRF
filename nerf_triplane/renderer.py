...
t6_6=0
zeroDepth = None
zero_amb_aud_sum = None
zero_amb_eye_sum = None
zero_uncertainty_sum = None
...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        if self.smooth_lips:
            self.enc_a = None
        #自定义参数，是否在运行评估模型，由外部评估模型时动态修改其值
        self.evaluate=False
    ...
    def run_cuda(self, rays_o, rays_d, auds, bg_coords, poses, eye=None, index=0, dt_gamma=0, bg_color=None,
        ...
        '''模型推理，最终调用到此方法！！！
            各环节累积用时(s):[t1=0.00938272476196289,
            t2=0.000110626220703125,
             t3=0.017627239227294922,
              t4=4.565474033355713,
               t5=0.0014884471893310547,
                t6=20.006036520004272,
                 t7=3.6812117099761963,
                  t8=0.00702977180480957,
                   t9=7.033348083496094e-05]
        '''
        ...
        begin = time.time()
        # encode audio
        # 这里是调用的network.py中的encode_audio方法
        ...
        if self.evaluate:
            ...
            pass
        begin = time.time()
        # 推理时self.training=False，训练是training=True
        if self.training:
            # setup counter
            ...
            while step < max_steps:
                ...
                global t6_3
                t6_3 += time.time() - begin

                begin = time.time()
                sigmas, rgbs, ambients_aud, ambients_eye, uncertainties = self(xyzs, dirs, enc_a, ind_code, eye)
                sigmas = self.density_scale * sigmas
                global t6_4
                t6_4 += time.time() - begin

                begin = time.time()
                # 源代码里面本来就注释了的：raymarching.composite_rays_uncertainty(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, ambients, uncertainties, weights_sum, depth, image, ambient_sum, uncertainty_sum, T_thresh)
                raymarching.composite_rays_triplane(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas,
                                                    ambients_aud, ambients_eye, uncertainties, weights_sum, depth,
                                                    image, amb_aud_sum, amb_eye_sum, uncertainty_sum, T_thresh)
                global t6_5
                t6_5 += time.time() - begin
        ...
        if self.training or self.evaluate:
        ...
        begin = time.time()
        # 外部程序实际上只用到了image，为节约运算时间，将非image的返回参数一律去掉！
        if self.training or self.evaluate:
            results['depth'] = depth
            results['ambient_aud'] = amb_aud_sum
            results['ambient_eye'] = amb_eye_sum
        ...
        return results, [f't1={t1}', f't2={t2}', f't3={t3}', f't4={t4}', f't5={t5}', f't6={t6}', f't7={t7}', f't8={t8}', f't9={t9}','后面是t6:',f't6_1={t6_1}',f't6_2={t6_2}',f't6_3={t6_3}',f't6_4={t6_4}',f't6_5={t6_5}',f't6_6={t6_6}']

    def run_cuda_for_inference(self, rays_o, rays_d, auds, bg_coords, poses, eye=None, index=0, dt_gamma=0, bg_color=None,
                 perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        '''推理时专用的方法！'''
        '''模型推理，最终调用到此方法！！！
            各环节累积用时(s):'t1=0.13267278671264648', 't2=0.0012326240539550781', 't3=0.20688986778259277', 't4=2.3814282417297363',
            't5=0.01787567138671875', 't6=0.49804139137268066', 't7=2.664675712585449', 't8=0.07786130905151367',
            't9=0.0009942054748535156', '后面是t6:', 't6_1=0.10087800025939941', 't6_2=0.028328418731689453', 't6_3=1.0091438293457031',
            't6_4=9.665000438690186', 't6_5=0.745232343673706', 't6_6=7.146045684814453'
        '''
        ...
        weights_sum = torch.zeros(N, dtype=dtype, device=device)
        global zeroDepth
        if zeroDepth is None:
            zeroDepth = torch.zeros(N, dtype=dtype, device=device)
        image = torch.zeros(N, 3, dtype=dtype, device=device)
        global zero_amb_aud_sum
        if zero_amb_aud_sum is None:
            zero_amb_aud_sum = torch.zeros(N, dtype=dtype, device=device)
        global zero_amb_eye_sum
        if zero_amb_eye_sum is None:
            zero_amb_eye_sum = torch.zeros(N, dtype=dtype, device=device)
        global zero_uncertainty_sum
        if zero_uncertainty_sum is None:
            zero_uncertainty_sum = torch.zeros(N, dtype=dtype, device=device)
        ...
        while step < max_steps:
            ...
            begin = time.time()
            # self(xyzs, dirs, enc_a, ind_code, eye):调用NeRFRenderer类中定义的__call__方法。在Python中，如果一个类定义了__call__方法，那么这个类的实例就可以像函数一样被调用
            sigmas, rgbs, ambients_aud, ambients_eye, uncertainties = self(xyzs, dirs, enc_a, ind_code, eye)
            sigmas = self.density_scale * sigmas
            global t6_4
            t6_4 += time.time() - begin
            ...
            # 源代码里面本来就注释了的：raymarching.composite_rays_uncertainty(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, ambients, uncertainties, weights_sum, depth, image, ambient_sum, uncertainty_sum, T_thresh)
            raymarching.composite_rays_triplane(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas,
                                                ambients_aud, ambients_eye, uncertainties, weights_sum, zeroDepth,
                                                image, zero_amb_aud_sum, zero_amb_eye_sum, zero_uncertainty_sum, T_thresh)
            #print(f'zero_amb_aud_sum={zero_amb_aud_sum}')
            global t6_5
            t6_5 += time.time() - begin
    ...
    def render(self, rays_o, rays_d, auds, bg_coords, poses, staged=False, max_ray_batch=4096, **kwargs):
        ...
        # 推理时staged=True,opt.cuda_ray=True
        if staged and not self.cuda_ray:
            ...
            # 判断是推理还是训练
            if self.training or self.evaluate:
                results, ts = self.run_cuda(rays_o, rays_d, auds, bg_coords, poses, **kwargs)
            else:
                # 推理
                results, ts = self.run_cuda_for_inference(rays_o, rays_d, auds, bg_coords, poses, **kwargs)
